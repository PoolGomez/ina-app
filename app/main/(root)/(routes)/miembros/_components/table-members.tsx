...
import { getCongregationName, getGroupName } from "@/lib/data";
...
const TableMembers = ({ data }: TableMembersProp) => {
  ...
  const sortedMembers = [...data].sort((a, b) => {
    ...
    const valueA = a[sortField as keyof Member].toString().toLowerCase();
    const valueB = b[sortField as keyof Member].toString().toLowerCase();
      ...
      switch (typeof value) {
        case "boolean":
          return value === true
            ...
            : "desactivada".includes(searchTerm.toLowerCase());
        case "string":
          if (value === member.grupo) {
            return getGroupName(value)
              .toLowerCase()
              .includes(searchTerm.toLowerCase());
          }
          if (value === member.congregacion) {
            return getCongregationName(value)
              .toLowerCase()
              .includes(searchTerm.toLowerCase());
          }
              ...
              >
                ...
                <div className="flex flex-col items-end">
                  <span className="text-lg font-bold">
                    {highlightText(
                      getCongregationName(member.congregacion),
                      searchTerm
                    )}
                  </span>

                  <span className="text-sm text-gray-500">
                    {highlightText(getGroupName(member.grupo), searchTerm)}
                  </span>
                </div>
              </div>
            </CellActionMobile>
